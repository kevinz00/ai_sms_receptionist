...
def append_message(conversation_id: str, role: str, text: str, source_message_id: str | None = None):
    """
...
def get_or_create_customer(business_id: str, customer_phone: str):
    
    pk = f"BUSINESS#{business_id}"
    sk = f"CUSTOMER#{customer_phone}"
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
            print("[INFO] Customer race detected, fetching existing")
            response = table.get_item(
                Key={"PK": pk, "SK": sk},
                ConsistentRead=True
            )
            ...
            if item:
                return item
            raise Exception("Customer race condition: item still missing")
        else:
...
def find_message_by_source_id(conversation_id: str, source_message_id: str):
    response = table.query(
        KeyConditionExpression=Key("PK").eq(f"CONVO#{conversation_id}")
    )
